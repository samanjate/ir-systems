...
public class iRFinalProject {
	...
	iRFinalProject indexer = null;
	try {
	    indexLocation = s;
	    indexer = new iRFinalProject(s);
	} catch (Exception ex) {
	...
	FileInputStream fstream = new FileInputStream("/Users/samanjatesood/Documents/cacm.queries.txt");
	...
	while ((strLine = br2.readLine()) != null) {
		...
		QueryParser parser = new QueryParser(Version.LUCENE_47, "contents",
			analyzer);
		Query q = parser.parse(QueryParser.escape(s));
		searcher.search(q, collector);
		ScoreDoc[] hits = collector.topDocs().scoreDocs;

		// 4. display results
		PrintWriter writer = new PrintWriter(new FileOutputStream(new File("cacm_queries_lucene.txt"),true));
		System.out.println("Found " + hits.length + " hits.");
		for (int i = 0; i < hits.length; ++i) {
		    int docId = hits[i].doc;
		    Document d = searcher.doc(docId);
		    String temp = d.get("path");
		    temp = temp.replace("/Users/samanjatesood/Desktop/IR/IR-samanjate-project/cacm/CACM-", "");
		    temp = temp.replace(".html", "");
		    System.out.println((i + 1) + ". " + d.get("path")
			    + " score=" + hits[i].score);
		    int rank = i+1;
		    writer.println(q_ID + " Q0 CACM-" + temp + " " + rank + " " + hits[i].score + " LUCENE");
		}
		writer.close();
     ...
     */
    iRFinalProject(String indexDir) throws IOException {
